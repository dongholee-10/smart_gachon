...
def analyze_company(request: CompanyAnalyzeRequest, db: Session = Depends(get_db)):
    try:
        return analyze_company_news(db=db, company=request.company, display=request.display)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def history(db: Session = Depends(get_db)):
    records = list_analysis_results(db)
